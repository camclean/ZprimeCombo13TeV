...
#
# For 1.a. iii., you can add those lines at the end of the nobs loop to calculate and print Z_w:
#Z_w = calc_Z_w(nobs, b)
#print "for nobs=%d: p=%.3g; Z=%.2f; Z_a=%.2f; Z_w=%.2f" % (nobs, p, Z, Z_a, Z_w)
...
def generate_poisson_unc(mu0, delta_mu, ntoy):
    ...
    for i in range(ntoy):
        ...
        # prior on b but a log-normal prior. This
        # can be implemented by generating a normally distributed number
        # theta with mean 0 and standard deviuation 1 and exp(theta * lambda) as
        # scale factor for mu0, where lambda is related to the uncertainty:
